...
preserves the earlier 64/80 motion run. Both describe the code before these
fixes.
...
channel must not be added twice. The energy report still leaves out
finite-step inductance and tire elastic storage.
...
- Stopped `boomerang` settles endpoint distance, then final heading,
...
| Targeted completion tests, tuned ideal plant | 27/27 pass |
...
4.3e-10 W. These count what was covered; they are not an accuracy percentage.

The new completion audit tests near-side, near-diagonal, behind, reverse and
endpoint-only-turn cases at 34/76/94 inches per second. With the stated
...
error. Drive encoders plus heading cannot observe lateral slip, and the
independent odometry oracle reproduces that limit. These failures stay in the
results. Nothing feeds ground truth into the controller to hide them.
...
and the driver's interleaved intake cadence, keeping both nine-block capacity
assertions.
...
resistance. The contact audit measures body impulse after replacing the
...
Passing the physics gate shows the tested equations, constraints and
specification envelopes hold. It does not calibrate anything to hardware. Motor equivalents, governor
...
cap is conservative and does not reproduce every published transient peak.
The 5.5 W thermal schedule is an assumption. IMU saturation and invalid flags,
and full-pack regeneration shunting, are simulation policies; they were not
taken from VEX firmware or protection circuits.
...
Tracking-wheel preloads are lower-bound biases on rigid supports; nobody
measured the spring forces. The offline Rapier intake prototype is unfinished
and does not run in the live physics engine. The interactive clock keeps its
250 ms catch-up cap for long host stalls.

Calibrating these parameters and tuning autonomous trajectories needs real-robot
logs and the robot's sensor and drivetrain configuration. With the remaining
motion failures, the tracking is not ready for competition.
...
The last three commands exit nonzero while their acceptance failures remain.
C++ oracle compiler commands are in the historical physics report. V5/ARM
compilation and hardware runs were not available for this pass.

Artifacts for this fix pass were written to `/tmp/vexsim-fixed-*.log`,
...
CSVs were generated in `/tmp`, and none of it is in the repository. No
dependencies, lockfiles, generated CAD or browser assets, or Git history were
changed. A graphify graph located dependencies; the source and independent
numerical checks determined the fixes.

## Installed source and recovery

The validated files were installed into `../vexsim`: 12 existing files updated
and five new regression files added. Every installed file was compared byte-for-byte with

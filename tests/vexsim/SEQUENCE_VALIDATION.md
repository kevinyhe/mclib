...
This pass also confirmed a **replay-inspection bug**. Selecting an earlier result
at a shared step-boundary timestamp displays the next action's commands and
target while the earlier action stays selected. Inspecting a skipped step shows
a previous action's frame without saying that the skipped step has no
recording. The browser reproduction has **one passing observation and three
failures**. No application code changed in this pass;
[DEBUGGER_VALIDATION.md](DEBUGGER_VALIDATION.md) records the later fix.

Running in order does not mean the motions succeed: **56/96 combinations
without injected failures** meet every motion check. The other **24** cases
inject timeouts on purpose or reject incompatible reverse-arc parameters.
...
near −87.890°. That extra rotation happens **before** the next action starts, so
it is a trajectory defect in the boomerang and the actions do not overlap.
...
3. The UI consequently shows **STEP 02 / MOTION**, **12/−12 V** and **177.89°**
   ...
   The real step-1 result is stopped, 1.389 inches / 2.110°.
...
playing. The UI needs to show these states differently, and editing a draft
must never change recorded data.
...
Most cases turn off stop-on-failure so later actions still run when an earlier
controller misses its endpoint. `timeout_abort` turns it on. Normal step
deadlines are four seconds, injected deadlines are 50 ms, and the locked
wall-reset deadline is 800 ms. PID defaults, voltages and tolerances were left
at their defaults.
...
to the previously recorded tracker run. The other presets and sensor layouts
still fail on deadline, heading or position, whether or not they run in order.

## State continuity and physical settlement

The runner constructs one bridge per sequence and reuses it for every step.
Calls are synchronous, and the adapter starts no background motion or
heading-control task. Each C++ routine constructs its own PID objects, while pose, sensor history,
...
during holds, and skip behavior. It makes 276,596 frame/event assertions, most
of them the same checks repeated across trace frames.

“Stopped” here means the controller commanded zero voltage or Hold. The body may
still be moving slightly:
...
The failed-step maxima occur in different cases. They show that continuing
after a failed motion starts the next one from a moving robot. For the user's
...
- A point move does not promise a final body heading. A following drive uses the
  measured heading left by that point move. Add a turn when the next drive must
...
- After a timeout, safety resets the held heading to the measured heading. With
  ...
  target. Its distance still uses encoder travel, so a more accurate pose does
  not remove distance errors from slip.
...
- **Sequence check / Timeout recovery**: fails its 50 ms turn on purpose;
...
exits 1 for its three confirmed failures. The builder tests pass 17/17, and the
reference-geometry tests pass 206 checks. The mixed-sequence script only fails
on scheduling problems, so its exit 0 does not mean every motion endpoint
passed.
...
one headless GUI skip, plus independent audits). This pass confirmed the source
matched that baseline; it did not rerun it. Reading the source and the recorded
runs established the sequencing results. No controller, simulator, frontend,
gains, dependencies or lockfiles changed. This is a simulation result. It does
not test firmware concurrency or measure hardware.

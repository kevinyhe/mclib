...
This change adds observation and playback. It does not change the controller or
the physics model. The native viewer is validated separately from motion
acceptance.

The artifact paths under `bin/` below are ignored by git and exist only on the
machine that ran the validation.
...
older recordings without those dimensions show a message asking for a rerun
instead of inventing a robot shape. Local Three.js assets are allowlisted; playback
...
- Native **Fit run** includes the recorded chassis geometry, mesh height and status
...
The blocking C++ controller ticks publish their phase, active heading
...
determinism. This did not involve a V5 firmware build or any hardware
...
corners and resizing are covered with the real projection code.

An independent real-browser check replayed all **631 frames** of the original
boomerang-point sequence at both 1366 × 768 and 390 × 844. The native scene's
projected body stayed inside the camera and clear of the status overlay
...
The pure JavaScript suite passes **206 geometry + 70 debugger + 21 handler
request-ownership checks**. The last group uses deferred transport stubs and
does not test physical motion. See
...
After the final physics-first gate, a fresh C++ boomerang-point run
...
distinct live timestamps were observed before completion; native scene
position and heading agreed with recorded physics. Telemetry followed the same time,
live scrubbing stayed disabled, pause/scrub and step-owned hold inspection worked,
and both endpoint checks passed with sequential action timing. The
approach-shape defect below is still open. See
...
dependencies were installed. Software WebGL rendered the native scene. These
checks verify playback and observation; they do not calibrate anything against
a robot.
...
Earlier attempts that ran on stale source or were interrupted stay recorded as
errors. The passing baseline does **not** mean the broader motion acceptance
matrix or the boomerang approach defect below is fixed.
...
`/tmp/`. No dependencies, lockfiles or vendored code changed, and the sibling
vexsim source was not modified; native assets are read in place.
...
The moving-carrot target itself passes -200°. Reconstructing its P+D request
matched recorded yaw voltage within 0.000980 V (millivolt quantization). The
cause is mostly the carrot geometry and how translation brakes. The robot is
not overshooting a fixed -90° target, and there is no derivative kick.
...
has not passed a regression matrix. Neither counts as a fix. See the
...
[SEQUENCE_VALIDATION.md](SEQUENCE_VALIDATION.md). The viewer and telemetry do
not change them.
